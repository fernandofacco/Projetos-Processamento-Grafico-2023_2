...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		const int quadradosPorLinhaColuna = 10;

		for (int i = 0; i < quadradosPorLinhaColuna; i++) {
			for (int j = 0; j < quadradosPorLinhaColuna; j++) {
				glm::mat4 model = glm::mat4(1); //Matriz identidade
				float time = (float)glfwGetTime();

				model = glm::translate(model, glm::vec3(i * width / quadradosPorLinhaColuna, j * height / quadradosPorLinhaColuna, 0.0));
				model = glm::scale(model, glm::vec3(width/2, height/2, 1.0));
				...
				shader.setVec4("inputColor", r, g, b, 1.0);
