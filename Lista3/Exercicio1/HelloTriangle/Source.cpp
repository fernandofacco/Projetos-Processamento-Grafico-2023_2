...
//#endif
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 1; i < 4 ; i++) {
			...
			if (i == 1) {
				model = glm::translate(model, glm::vec3(200 * i, 150.0 * i, 0.0));
				model = glm::rotate(model, time * 2, glm::vec3(0.0, 0.0, 1.0));
				model = glm::scale(model, glm::vec3(100.0, 100, 1.0));
			}
			else if (i == 2) {
				model = glm::translate(model, glm::vec3(200 * i, 150.0 * i, 0.0));
				model = glm::rotate(model, glm::radians(45.0f), glm::vec3(0.0, 0.0, 1.0));
				model = glm::scale(model, glm::vec3(100.0 * (time * 0.1), 100 * (time * 0.1), 1.0));
			}
			else {
				model = glm::translate(model, glm::vec3((200 * i) * (time * 0.1), 150.0 * i, 0.0));
